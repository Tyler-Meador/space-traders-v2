...
if 'activeShip' in st.session_state:
    st.write(f"Active Ship: {st.session_state['activeShip']["symbol"]} -> {st.session_state['activeShip']['registration']['role']}")
            ...
            with st.container():
                ...
                if hasattr(response, 'keys'):
                    for key in response.keys():
                        expander = st.expander(label=(f"{key} - {response[key]["type"]}"))
                        with expander:
                            ...
                            if len(response[key]["traits"]) <= 0:
                                ...
                                for trait in response[key]["traits"]:
                                    with st.container(border=True):
                                        ...
                                        st.write("Description: " + trait["description"])
